...
//ATENÇÃO QUE ISTO É NOVO
bool aux1 = false;
bool aux0 = false;

void callbackFunc0 (const geometry_msgs::PoseWithCovarianceStamped& msg)
{
   if (abs (msg.pose.pose.position.x - 6) < 0.5) {
	  if (abs (msg.pose.pose.position.y - 2) < 0.5) {
		 aux0 = true;
	  }
   }
}
...
//ATENÇÃO QUE ISTO É NOVO
void waitforrobot0 (string params, bool *run) {
	
	ros::NodeHandle nh;		
	//ros::Subscriber sub = n.subscribe("/robot2/amcl_pose", 1,callbackFunc1);
	ros::Subscriber sub = nh.subscribe("/robot_0/amcl_pose", 1,callbackFunc0);
...
//ATENÇÃO QUE ISTO É NOVO
void callbackFunc1 (const geometry_msgs::PoseWithCovarianceStamped& msg)
{
   if (abs (msg.pose.pose.position.x - 4) < 0.5) {
	  if (abs (msg.pose.pose.position.y - 2) < 0.5) {
		 aux1 = true;
	  }
   }
...
//ATENÇÃO QUE ISTO É NOVO
void waitforrobot1 (string params, bool *run) {
	
	ros::NodeHandle nh;
	//ros::Subscriber sub = n.subscribe("/robot2/amcl_pose", 1,callbackFunc1);
	ros::Subscriber sub = nh.subscribe("/robot_1/amcl_pose", 1,callbackFunc1);
